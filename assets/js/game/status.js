    ...
    if (!game.started) {
        if (Object.entries(game.players).length >= game.seeker_count + 1) {
            if (game.host == game.username) {
                status_box.innerHTML = `<button onmouseup="startGame()" ontouchstart="startGame()">Start Game</button>${game.abilityCooldown || you.teleporting ? "<br>" : ""}<br>Invite players: <a href="#" onclick="copyPlayLink()">${location.protocol}//${document.location.hostname}/play?region=${WS_HOST}&room=${game.room}</a>${game.abilityCooldown ? `<br>Ability cooldown: <b>${Math.round((game.abilityCooldown - Date.now()) / 100) / 10}</b> seconds left.` : ""}${you.teleporting ? `<br>You will be teleported in <b>${Math.ceil(you.teleporting / 1000)}</b> seconds.` : ""}`;
            } else {
                status_box.innerHTML = `Waiting for host to start the game...<br>Invite players: <a href="#" onclick="copyPlayLink()">${location.protocol}//${document.location.hostname}/play?region=${WS_HOST}&room=${game.room}</a>${game.abilityCooldown ? `<br>Ability cooldown: <b>${Math.round((game.abilityCooldown - Date.now()) / 100) / 10}</b> seconds left.` : ""}${you.teleporting ? `<br>You will be teleported in <b>${Math.ceil(you.teleporting / 1000)}</b> seconds.` : ""}`;
            }
        } else {
            status_box.innerHTML = `Waiting for players to join...<br>Invite players: <a href="#" onclick="copyPlayLink()">${location.protocol}//${document.location.hostname}/play?region=${WS_HOST}&room=${game.room}</a>${game.abilityCooldown ? `<br>Ability cooldown: <b>${Math.round((game.abilityCooldown - Date.now()) / 100) / 10}</b> seconds left.` : ""}${you.teleporting ? `<br>You will be teleported in <b>${Math.ceil(you.teleporting / 1000)}</b> seconds.` : ""}`;
        }
    } else {
        const you = game.players[game.username];
        switch(you.role) {
